...
export function WhatsappTemplateCard({
  ...
  const isApproved = template.approval === "aprovado";
          ...
          {isApproved ? (
            <Tooltip>
              <TooltipTrigger asChild>
                <span className="flex size-11 items-center justify-center rounded-lg text-muted-foreground/50 sm:size-8">
                  <Lock className="size-4" />
                </span>
              </TooltipTrigger>
              <TooltipContent>
                Template aprovado pela Meta não pode ser editado. Duplique e envie para nova
                aprovação.
              </TooltipContent>
            </Tooltip>
          ) : (
            <button
              type="button"
              aria-label={`Editar rascunho ${template.name}`}
              onClick={onEdit}
              className="flex size-11 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground sm:size-8"
            >
